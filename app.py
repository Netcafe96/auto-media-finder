...
# Helper functions
def get_youtube_links(q, max_results=5):
    try:
...
def get_tiktok_links(q, max_results=5):
    try:
        q_enc = urllib.parse.quote_plus(q)
        url = f"https://www.tiktok.com/search?q={q_enc}"
        headers = {"User-Agent": "Mozilla/5.0"}
        r = requests.get(url, headers=headers)
...
def get_google_image_previews(q, max_images=5):
    try:
        q_enc = urllib.parse.quote_plus(q)
        ...
        img_urls = list(set([
            line.split('src="')[1].split('"')[0] for line in r.text.split('\n')
            if 'mimg' in line and 'src="' in line
        ]))
