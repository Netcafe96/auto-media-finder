...
def get_youtube_video_data(q, max_results=5): try: q_enc = urllib.parse.quote_plus(q + " short") url = f"https://www.youtube.com/results?search_query={q_enc}" headers = {"User-Agent": "Mozilla/5.0"} r = requests.get(url, headers=headers) results = [] seen = set() for line in r.text.split('\n'): if 'watch?v=' in line: try: vid = line.split('watch?v=')[1].split('\')[0].split('"')[0] if vid not in seen: video_url = f"https://www.youtube.com/watch?v={vid}" thumbnail_url = f"https://img.youtube.com/vi/{vid}/0.jpg" results.append((video_url, thumbnail_url)) seen.add(vid) if len(results) >= max_results: break except: continue return results except: return []

def get_google_image_previews(q, max_images=15): try: q_enc = urllib.parse.quote_plus(q) url = f"https://www.bing.com/images/search?q={q_enc}&form=HDRSC2&first=1&tsc=ImageBasicHover" headers = {"User-Agent": "Mozilla/5.0"} r = requests.get(url, headers=headers, timeout=5) img_urls = list(set([ line.split('src="')[1].split('"')[0] for line in r.text.split('\n') if ('mimg' in line or 'imgres' in line) and 'src="' in line and ('.jpg' in line or '.png' in line) ])) return img_urls[:max_images] if img_urls else None except: return None
